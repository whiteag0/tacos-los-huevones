...
export default function MenuCard({ item }: MenuCardProps) {
  ...
  const requiresVariant = hasVariants && item.variants?.required;

  // Calculate total price including variant price modifier
  const unitPrice = item.price + (selectedVariant?.price_modifier || 0);
  const totalPrice = unitPrice * quantity;

  const handleSelectVariant = (option: MenuVariantOption) => {
    setSelectedVariant({
      id: option.id,
      name: option.name,
  ...
  const handleAddToCart = () => {
    ...
    addToCart(item, quantity, specialInstructions || undefined, selectedVariant || undefined);
    setShowModal(false);
    setQuantity(1);
  ...
  const handleQuickAdd = () => {
    // If item has required variants, open modal instead of quick add
    if (requiresVariant) {
      setShowModal(true);
      return;
    }
  ...
  return (
    <>
      <div className="group bg-white rounded-2xl overflow-hidden card-hover border border-gray-100 h-full flex flex-col">
        {/* Image Container - shorter on mobile */}
        <div className="relative aspect-[16/10] sm:aspect-[4/3] bg-gradient-to-br from-orange-50 to-red-50 overflow-hidden flex-shrink-0">
          {item.image_url ? (
            <Image
              src={item.image_url}
              alt={item.name}
          ...
          {/* Price Badge */}
          <div className="absolute top-2 right-2 sm:top-3 sm:right-3 bg-white/95 backdrop-blur-sm text-gray-900 font-bold px-2 py-1 sm:px-3 sm:py-1.5 rounded-full shadow-lg text-xs sm:text-sm">
          ...
          <div className="absolute top-2 left-2 sm:top-3 sm:left-3 flex flex-col gap-1.5 sm:gap-2">
            {item.is_popular && (
              <span className="bg-gradient-to-r from-amber-400 to-orange-400 text-white text-[10px] sm:text-xs font-semibold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full shadow-sm flex items-center gap-1">
            ...
            {item.is_vegetarian && (
              <span className="bg-gradient-to-r from-green-500 to-emerald-500 text-white text-[10px] sm:text-xs font-semibold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full shadow-sm flex items-center gap-1">
                <svg className="w-2.5 h-2.5 sm:w-3 sm:h-3" fill="currentColor" viewBox="0 0 20 20">
          ...
          <button
            ...
            disabled={isAdding}
            className={`absolute bottom-2 right-2 sm:bottom-3 sm:right-3 bg-red-600 text-white w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center shadow-lg transition-all duration-300 btn-press ${
              isAdding
        ...
        <div className="p-3 sm:p-4 flex flex-col flex-grow">
          <div className="flex justify-between items-start gap-2 mb-1 sm:mb-2">
            <h3 className="font-semibold text-gray-900 leading-tight text-sm sm:text-base">{item.name}</h3>
          </div>

          <p className="text-gray-500 text-xs sm:text-sm mb-2 sm:mb-3 line-clamp-2 leading-relaxed flex-grow">{item.description}</p>
          ...
          <div className="flex items-center justify-between pt-2 border-t border-gray-100 mt-auto">
            ...
            <button
              ...
              className="text-red-600 hover:text-red-700 font-medium text-xs sm:text-sm flex items-center gap-1 transition-colors"
            >
              {hasVariants ? 'Select Options' : 'Customize'}
            ...
            <div className="p-6">
              <div className="flex justify-between items-start mb-2">
                ...
                <span className="text-red-600 font-bold text-xl">
                  ${item.price.toFixed(2)}{hasVariants && '+'}
                </span>
              </div>
              ...
              {hasVariants && item.variants && (
                <div className="mb-5">
                  <label className="block text-sm font-semibold text-gray-700 mb-3">
                    {item.variants.label}
                    {item.variants.required && <span className="text-red-500 ml-1">*</span>}
                  </label>
                  <div className="grid grid-cols-2 gap-2">
                    {item.variants.options.map((option) => (
                      <button
                        key={option.id}
                      ...
                      >
                        ...
                        <div className="text-sm text-gray-500">{option.name_en}</div>
              ...
              <button
                ...
                className={`w-full py-4 rounded-xl font-bold text-lg transition-all shadow-lg btn-press ${
              ...
              >
                {requiresVariant && !selectedVariant
                  ? `Select ${item.variants?.label?.toLowerCase() || 'option'}`
